...
async def new_task_subject(message: types.Message, state:FSMContext):
    try:
        ...
        await bot.send_message(chatid, 'Напиши текст домашнего задания')
...
async def new_task(message: types.Message, state:FSMContext):
    try:
        ...
        db.add_task(state_data['date'], state_data['subject'], task_text)
...
async def callback(call: types.CallbackQuery, state:FSMContext):
    try:
        ...
        if call.data == 'marks':
            ...
            await bot.edit_message_text(task_text, chatid, messageid, reply_markup=nav.back_to_dates)
    except Exception as e:
        await err(e, chatid)
